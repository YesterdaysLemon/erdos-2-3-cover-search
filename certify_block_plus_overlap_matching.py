#!/usr/bin/env python3
"""Combine a verified anchor-block loss with disjoint pair intersections."""
...
def main() -> int:
    ...
    for weight, _first, _second, left, right in edges:
        ...
        if total_density - block_loss - pair_overlap < 1:
            break
    total_loss = block_loss + pair_overlap
    ...
    result = {
        ...
        "selected_disjoint_pairs": pairs,
        "pair_overlap_sum": fraction_payload(pair_overlap),
    ...
    print(
        f"anchors={len(anchor_primes)} pairs={len(pairs)} "
        f"block_loss={block_loss} pair_loss={pair_overlap} "
