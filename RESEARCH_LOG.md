...
## Period-330,442,912,800 elimination (2026-07-26)

The projected-conditional design search was next applied to the strongest
developed block for this 806-row family.  The permanent 16-anchor block
promotes
...
through four independently replayed conditional extensions.

The highest-value automated conditional improvements occurred at

```
p = 41, 43, 6553, 151, 73, 911, 34511, 241, 281, 337.
```

Each was regenerated by the dedicated projected-conditional certificate
program and independently replayed with `verified=True`.  Three of these
replace weaker hand-built conditional edges; the remaining seven strengthen
previously generic star edges.  Together with the other retained
conditionals, the exact 21-edge star bound is

```
2599707959748681404489 / 2599821871898771520000
= 0.9999561846327545 < 1.
```

The exact no-cover margin is

```
113912150090115511 / 2599821871898771520000
= 0.0000438153672454952.
```

The independent period replay checked all 16 block anchors, 790 outside
rows, and all 21 conditional dependencies and reported `verified=True`.
This eliminates period `330442912800` from the ranked finite menu.
...
## Period-3,139,207,671,600 overlap and constructive checkpoint (2026-07-26)

The closest ranked survivor was developed from a 20-anchor block by
promoting
...
into a 24-anchor block.  With at most the 20 smallest compatible anchors on
each star edge, the exact exploratory period bound before new conditional
designs was

```
64700915810477888019943 / 64409312453805839520000
= 1.0045273477632786.
```

The anchor-limit option is proof-safe: it merely chooses a subunion when
lower-bounding an outside-to-block intersection.  The generator records the
limit, and the independently written period verifier reconstructs the same
compatible subset using its own target-map index calculation.  A complete
period-`330442912800` roundtrip with limit 10 replayed with `verified=True`.

Several bounded attempts did not close the new family:

* promoting nine existing low-modulus conditional rows into a 33-anchor
  block and using an 18-anchor star limit gave `1.0082428409982556`;
* a sixth-order Bonferroni search through outside-plus-six-anchor systems
  did not beat the existing bounds on the nine highest-slack helper rows;
* a meet-in-the-middle quadratic optimizer selected pairwise Bonferroni
  subsets from all 24 anchors on all 1,007 outside rows.  It found 46 strict
  improvements with raw total about `0.0000212052`, but most were dominated
  by stronger projected designs.

The projected design search was extended in three ways:

1. it writes a valid JSON checkpoint after every completed outside fibre and
   can resume a matching checkpoint;
2. it can contract one projected anchor together with an independent line
   on the same residual component;
3. it enforces that a shared residual is coprime to the base period, matching
   the dedicated certificate generator's structural contract.

The third item corrected an important discovery error.  An initial automated
design paired `p=17` with `p=5` on residual 4 while its base period was
divisible by 4, producing an invalid apparent improvement.  That run was
stopped and discarded before certification.  A regression test now rejects
this structure, while a valid coprime shared-residual toy system still
matches complete phase enumeration.

After the correction, 272 distinct outside fibres had completed projected
screening.  Taking the strongest valid discovery edge per outside prime,
and the stronger meet-in-the-middle edge only where it wins, gives total
exploratory improvement

```
12411163622144643908413 / 3284874935144097815520000
= 0.0037782758452568612.
```

Subtracting these distinct star-edge improvements gives

```
11740484081115098732531 / 11731696196943206484000
= 1.0007490719180216.
```

The remaining excess above one is

```
8787884171892248531 / 11731696196943206484000
= 0.00074907191802171.
```

This composition as a whole is discovery-only.  The winning projected
designs have not all been regenerated by the dedicated certificate program
or replayed by its independent verifier, and the bound is above one in any
case.

The strongest individual paired edge, for outside prime `6553`, was promoted
to a permanent proof artifact:

```
order_pool_1050000_period3139207671600_conditional_fibre6553_paired_autodesign_certificate.json
order_pool_1050000_period3139207671600_conditional_fibre6553_paired_autodesign_verification.json
```

The independent verifier exhaustively replayed all `1,990,656` target
combinations and returned `verified=True`, with forced intersection
...
This certifies that one edge only.  It does not promote the aggregate bound
or eliminate period `3139207671600`.

The positive search was also restarted on this 1,031-row family.  A compact
multivalued CEGIS run reached a persisted 35,492-witness checkpoint after
198 total rounds across its initial and continued stages.  Repair time on
the final unrestricted rounds rose from 24 to 37 to 74 seconds, while exact
checking still returned 180-200 uncovered points.  Two independently seeded
component-repair runs each ended after 120 rounds with 21,372 witnesses.
Point-wise and coordinate-descent variants each ended after 80 rounds with
14,248 witnesses.  Every finite repair succeeded, but every exact checker
retained holes.  These are constructive null results, not UNSAT
certificates.

The test suite now contains 42 passing tests, including complete small-group
enumeration for the shared-residual contraction, a regression for the
coprimality guard, exhaustive small quadratic subset comparisons, and an
end-to-end checkpoint/resume smoke test.

The certified public frontier remains 4,617 of 4,637 ranked finite families
eliminated, with 20 remaining.  There is still no integer `m`, no complete
finite cover, and no global impossibility proof.

## Period-3,139,207,671,600 certified elimination (2026-07-26)

The preceding discovery-only checkpoint was closed by strengthening four
conditional block extensions.  Dedicated projected-conditional certificates
and independently written verifiers established the following improvements
over the earlier extension bounds:

```
outside p=41:  23/177650 - 374873/37093320
outside p=103: 824473/194739930 - 42787/10249470
outside p=601: 1028851/139099950 - 839/118800
outside p=191: 11587/1909215 - 659/112860
```

Their total exact block-loss improvement is

```
5863349 / 7789597200
= 0.0007527153008630536.
```

The decisive `p=41` certificate exhaustively checks `11,059,200` target
combinations.  The other three check `2,764,800`, `3,870,720`, and
`3,870,720` combinations respectively.  All four independent reports return
`verified=True`.  The unchanged `p=31`, `p=43`, and `p=73` extensions were
also promoted or reused with independent reports, and the complete
17-to-24-anchor extension chain replays at every step.

Every strongest valid projected design from the 37 corrected discovery
batches was then regenerated by
`promote_projected_conditional_designs.py`.  The known-invalid shared-residual
batch is excluded by basename and, independently, by the generator's
coprimality contract.  All 165 distinct winning outside fibres reproduced
their exact discovery fractions and passed the independent projected-fibre
verifier.  Their total improvement over the earlier star baselines is

```
12407842119878078171429 / 3284874935144097815520000
= 0.003777264694959774.
```

The top-level star no longer needs to optimize through `2^20` subsets for
each outside row.  The earlier meet-in-the-middle scan supplied one explicit
compatible anchor subset for every one of the 1,007 outside rows.  A new
proof-producing witness generator recomputes each subset's exact pairwise
Bonferroni value; a separately written verifier recomputes all 1,007 values
using independent target-map index code and returns `verified=True`.
Optimality of the selected subsets is not asserted or needed.  On 959 rows
the witnesses equal the old exhaustive baseline, on 46 they improve it, and
on two they are weaker; both weaker rows (`p=101,127`) are overridden by
stronger replayed conditional certificates.

The assembled 1,031-row certificate is

```
order_pool_1050000_period3139207671600_conditional_star_certificate.json
```

Its independent top-level replay checks the 24-row block, all 165
conditional reports, all 1,007 explicit baseline witnesses, and obtains

```
11731653453882582685001 / 11731696196943206484000
= 0.9999963566171587.
```
...
```
42743060623798999 / 11731696196943206484000
= 0.000003643382841343613.
```

The replay report returns `verified=True`, so period `3139207671600` is
eliminated from the ranked finite menu.  The certified frontier is now 4,618
of 4,637, with 19 remaining.
...
## Period-14,440,355,289,360 certified elimination (2026-07-26)

The next ranked survivor has 1,124 rows and period components

```
16, 27, 5, 7, 11, 13, 17, 19, 23, 29, 31.
```

Starting from the existing 14-anchor block, five independently replayed
conditional extensions promoted `p=31`, `p=37`, `p=73`, `p=41`, and `p=43`.
The resulting 19-anchor block has exact forced overlap loss

```
377312154515 / 1064026179216
= 0.3546079616133248
```

and block-union upper bound

```
9082263823 / 12091206582
= 0.7511461954938915.
```

Combining this block with 33 previously replayed conditional edges and the
old star baselines still gave

```
5540989535211663349591 / 5517818022358427385600
= 1.0041993977980688,
```

so the enlarged block alone did not eliminate the family.

Six projected-design discovery batches completed 159 outside fibres.  After
deduplicating by outside prime, 129 designs strictly improved their current
baselines.  Every selected design was regenerated by the dedicated
certificate program and replayed by the independently written projected-
fibre verifier.  The promotion manifest records 129 of 129 successful
replays and total exact improvement

```
54110319599866426801 / 12791305415467263484800
= 0.0042302421717205.
```

Twenty-six promoted outside primes overlap the earlier conditional set.
Keeping the stronger promoted certificate on each overlap leaves 136
distinct conditional edges in the assembled proof.

A proof-producing pairwise-star pass also records one explicit compatible
anchor subset for every one of the 1,105 rows outside the block.  The
separate witness verifier recomputed all subset bounds with exact rational
arithmetic and returned `verified=True`.  As before, the witnesses need not
be optimal: a replayed conditional edge replaces the baseline whenever it is
stronger.

The assembled certificate is

```
order_pool_1050000_period14440355289360_conditional_star_certificate.json
```

Its independent top-level replay checks the 19-row block, 136 distinct
conditional reports, and all 1,105 explicit star witnesses, obtaining

```
12234784315852076932153 / 12235161701751295507200
= 0.9999691556263482.
```
...
```
377385899218575047 / 12235161701751295507200
= 0.00003084437365176444.
```

The replay returns `verified=True`, so period `14440355289360` is eliminated
from the ranked finite menu.  The certified frontier is now 4,619 of 4,637,
with 18 remaining.
